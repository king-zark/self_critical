...
class AttModel(CaptionModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        pre_att_res_1 = Variable(torch.zeros(80, 1500), requires_grad=True).cuda()
        pre_att_res_2 = Variable(torch.zeros(80, 1500), requires_grad=True).cuda()
        ...
        for i in range(seq.size(1) - 1):
            ...
            output, state, pre_att_res_1, pre_att_res_2 = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, pre_att_res_1, pre_att_res_2)
            outputs[:, i] = output
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, pre_att_res_1=None, pre_att_res_2=None):
        ...
        output, state, pre_att_res_1, pre_att_res_2 = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks, pre_att_res_1, pre_att_res_2)
        logprobs = F.log_softmax(self.logit(output), dim=1)

        return logprobs, state, pre_att_res_1, pre_att_res_2
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                ...
                logprobs, state, pre_att_res_1, pre_att_res_2 = self.get_logprobs_state(it, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, state, pre_att_res_1, pre_att_res_2)
    ...
    def _sample_bak(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            if sample_max:
                ...
                sampleLogprobs2 = logprobs2.gather(1, it2) # gather the logprobs at sampled positions
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        batch_size = fc_feats.size(0)
        state = self.init_hidden(batch_size)
        pre_att_res_1 = Variable(torch.zeros(batch_size, 1500), requires_grad=True).cuda()
        pre_att_res_2 = Variable(torch.zeros(batch_size, 1500), requires_grad=True).cuda()
...
class DenseAttCore_multiATT(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None, pre_att_res_1=None, pre_att_res_2=None):
        ...
        h_1, state_1 = self.lstm1(torch.cat([h_0,att_res_1],1), [state[0][1:2], state[1][1:2]])
        att_res_2 = self.att2(torch.cat([h_1 + self.emb2(att_res_1), pre_att_res_2],1), att_feats, p_att_feats, att_masks)
        ...
        h_2, state_2 = self.lstm2(torch.cat([self.fusion1(torch.cat([h_0, h_1], 1)),att_res_2],1), [state[0][2:3], state[1][2:3]])
        return self.fusion2(torch.cat([h_0, h_1, h_2], 1)), [torch.cat(_, 0) for _ in zip(state_0, state_1, state_2)], pre_att_res_1, pre_att_res_2
