...
from .CaptionModel_multi_a import CaptionModel
...
class AttModel(CaptionModel):
    ...
    def init_hidden(self, bsz):
        ...
        return (weight.new_zeros(self.num_layers+1, bsz, self.rnn_size),
                ...
                weight.new_zeros(self.num_layers, bsz, self.att_hid_size))
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        word_feats = Variable(torch.zeros(batch_size, 41, self.rnn_size), requires_grad=False).cuda()
        # word_masks = Variable(torch.zeros(batch_size, self.seq_length), requires_grad=True).cuda()
        ...
        for i in range(seq.size(1) - 1):
            ...
            output, output2, state, pre_att_res_1, pre_att_res_2, word_feats = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, pre_att_res_1, pre_att_res_2, word_feats, i)
            outputs[:, i] = output
            outputs2[:, i] = output2

        return (outputs, outputs2)

    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, pre_att_res_1=None, pre_att_res_2=None, word_feats=None, iter_num=0):
        # 'it' contains a word index
        xt = self.embed(it)

        output, output2, state, pre_att_res_1, pre_att_res_2, word_feats = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks, pre_att_res_1, pre_att_res_2, word_feats, iter_num)
        logprobs = F.log_softmax(self.logit(output), dim=1)
        logprobs2 = F.log_softmax(self.logit2(output2), dim=1)

        return logprobs, logprobs2, state, pre_att_res_1, pre_att_res_2, word_feats
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for k in range(batch_size):
            pre_att_res_1 = Variable(torch.zeros(beam_size, self.rnn_size), requires_grad=True).cuda()
            pre_att_res_2 = Variable(torch.zeros(beam_size, self.rnn_size), requires_grad=True).cuda()
            word_feats = Variable(torch.zeros(beam_size, 41, self.rnn_size), requires_grad=False).cuda()
            ...
            for t in range(1):
                ...
                logprobs, logprobs2, state, pre_att_res_1, pre_att_res_2, word_feats = self.get_logprobs_state(it, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, state, pre_att_res_1, pre_att_res_2, word_feats, 0)
            logprobs = logprobs + 0*logprobs2
            self.done_beams[k] = self.beam_search(state, logprobs, pre_att_res_1, pre_att_res_2, word_feats, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, opt=opt)
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, logprobs2, state, pre_att_res_1, pre_att_res_2, word_feats = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, pre_att_res_1, pre_att_res_2, word_feats, t)
...
class TopDownCore(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        self.attention = Attention(opt)
        self.attention_word = Attention_word(opt)
        # self.word_masks = Variable(torch.zeros(opt.batch_size*5, 41), requires_grad=False).cuda()

    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None, pre_att_res_1=None, pre_att_res_2=None, word_feats=None, iter_num = 0):
        prev_h = state[0][-1]
        att_lstm_input = torch.cat([prev_h ,fc_feats, xt], 1)
        word_masks = Variable(torch.zeros(word_feats.size()[0], 41), requires_grad=False).cuda()
        word_masks[:,:iter_num] = 1
        h_att, c_att = self.att_lstm(att_lstm_input, (state[0][0], state[1][0]))
        # h_att = F.dropout(h_att, self.drop_prob_lm, self.training)
        att = self.attention(torch.cat([h_att,pre_att_res_1], 1), att_feats, p_att_feats, att_masks)
        att_word = self.attention_word(torch.cat([h_att,], 1), word_feats, word_masks)
        lang_lstm_input = torch.cat([h_att, att, att_word], 1)
        # lang_lstm_input = torch.cat([att, F.dropout(h_att, self.drop_prob_lm, self.training)], 1) ?????
        h_lang, c_lang = self.lang_lstm(lang_lstm_input, (state[0][1], state[1][1]))
        pre_att_res_1 = att.clone()
        pre_att_res_1 = pre_att_res_1.detach()
        ...
        output = F.dropout(h_lang, self.drop_prob_lm, self.training)
        state1 = (torch.stack([h_att, h_lang]), torch.stack([c_att, c_lang]))
        word_feats_ = word_feats.clone().detach()
        
        # word_feats_.requires_grad = False
        
        word_feats_[:,iter_num] = h_lang
        # word_masks_[:,iter_num] = 1
        
        word_feats = word_feats_
        # for the second output
        # att_lstm_input = torch.cat([fc_feats, prev_h], 1)
        ...
        return output, output, state1, pre_att_res_1, pre_att_res_2, word_feats

class TopDownCore_LSTM(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(TopDownCore_LSTM, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm

        self.att_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size) # we, fc, h^2_t-1
        # self.lang_lstm = nn.LSTMCell(opt.rnn_size * 2, opt.rnn_size) # h^1_t, \hat v
        self.fusion = nn.Sequential(nn.Linear(opt.rnn_size*3, opt.rnn_size), nn.ReLU())
        self.attention = Attention_LSTM(opt)
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None, pre_att_res_1=None, pre_att_res_2=None):
        ...
        h_att, c_att = self.att_lstm(att_lstm_input, (state[0][0], state[1][0]))
        h_att = F.dropout(h_att, self.drop_prob_lm, self.training)
        att, state_ = self.attention(torch.cat([h_att, pre_att_res_1], 1), att_feats, p_att_feats, (state[2][0],state[2][1]), att_masks)
        # pre_att_res_1 = att
        pre_att_res_1 = att.clone()
        pre_att_res_1 = pre_att_res_1.detach()
        pre_att_res_1.requires_grad = False
        tmp_h = h_att.clone()
        tmp_h = tmp_h.detach()
        tmp_h.requires_grad = False
        lang_lstm_input = torch.cat([tmp_h, pre_att_res_1, xt], 1)
        # lang_lstm_input = torch.cat([att, F.dropout(h_att, self.drop_prob_lm, self.training)], 1) ?????
        h_lang, c_lang = self.att_lstm(lang_lstm_input, (state[0][1], state[1][1]))
        h_lang = self.fusion(torch.cat([h_att, h_lang, att], 1))
        output = F.dropout(h_lang, self.drop_prob_lm, self.training)
        state = (torch.stack([h_att, h_lang]), torch.stack([c_att, c_lang]), state_)

        # att_ = self.fusion(torch.cat([att, h_att],1))
        # output = F.dropout(att_, self.drop_prob_lm, self.training)
        # state = (torch.stack([h_att, h_att]), torch.stack([c_att, c_att]), state_)
...
class Attention_word(nn.Module):
    def __init__(self, opt):
        super(Attention_word, self).__init__()
        self.rnn_size = opt.rnn_size
    ...
    def forward(self, h, att_feats, att_masks=None):
        ...
        weight = F.softmax(dot, dim=1)                             # batch * att_size
        if att_masks is not None and torch.sum(att_masks)>0:
            weight = weight * att_masks.view(-1, att_size).float()
            weight = weight / weight.sum(1, keepdim=True) # normalize to 1
        att_feats_ = att_feats.view(-1, att_size, att_feats.size(-1)) # batch * att_size * att_feat_size
        att_res = torch.bmm(weight.unsqueeze(1), att_feats_).squeeze(1) # batch * att_feat_size
...
class Attention_LSTM(nn.Module):
    def __init__(self, opt):
        ...
        self.att_hid_size = opt.att_hid_size
        opt_ = deepcopy(opt)
        opt_.input_encoding_size = self.rnn_size*2
        opt_.rnn_size = self.att_hid_size
        self.h2att = nn.LSTMCell(opt.rnn_size * 2, opt.att_hid_size)
        # self.h2att = nn.Linear(self.rnn_size*2, self.att_hid_size)
        self.alpha_net = nn.Linear(self.att_hid_size, 1)

    def forward(self, h, att_feats, p_att_feats, state, att_masks=None):
        # print(state.size())
        # The p_att_feats here is already projected
        att_size = att_feats.numel() // att_feats.size(0) // att_feats.size(-1)
        att = p_att_feats.view(-1, att_size, self.att_hid_size)
        att_h, state_ = self.h2att(h, state)                    # batch * att_hid_size
        state_ = torch.stack([att_h, state_])
        att_h = att_h.unsqueeze(1).expand_as(att)            # batch * att_size * att_hid_size
        dot = att + att_h                                   # batch * att_size * att_hid_size
        dot = F.tanh(dot)                                # batch * att_size * att_hid_size
        dot = dot.view(-1, self.att_hid_size)               # (batch * att_size) * att_hid_size
...
class DenseAttModel(AttModel):
    def __init__(self, opt):
        ...
        self.num_layers = 3
        self.core = DenseAttCore_multiATT(opt)
